...
"""
        node = markdown_to_html_node(md)
        html = node.to_html()
        self.assertEqual(html, "<div><pre><code>print('hi')</code></pre></div>")

    def test_links(self):
        md = "Here is a [link to boot.dev](https://boot.dev)"
        node = markdown_to_html_node(md)
        html = node.to_html()
        self.assertEqual(
            html,
            '<div><p>Here is a <a href="https://boot.dev">link to boot.dev</a></p></div>',
        )
...
"""
    ...
    def test_images(self):
        md = "Here is an ![image](https://i.imgur.com/zjjcJKZ.png)"
        node = markdown_to_html_node(md)
        html = node.to_html()
        self.assertEqual(
            html,
            '<div><p>Here is an <img src="https://i.imgur.com/zjjcJKZ.png" alt="image"/></p></div>',
        )
...
"""
        node = markdown_to_html_node(md)
        html = node.to_html()
        self.assertEqual(html, "<div><blockquote>hello world</blockquote></div>")
